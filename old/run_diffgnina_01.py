...
def run_diffdock(protein_path, smiles_list, n_poses):
    ...
    # 改善点1: tempfileを使って一時的なYAMLファイルを作成し、競合を防止
...
# ==========================================
# 2. GNINAによる評価とフィルタリング (並列処理用ワーカー関数)
# ==========================================
def _evaluate_single_pose(args):
    ...
    # モデルConfによる事前足切り（不要なGNINA実行をスキップして高速化）
    if conf < min_conf:
        return None
    ...
    # ▼ 改善点3: GNINA自体がエラー終了(OOMなど)した場合のハンドリング
    if proc.returncode != 0:
        print(f"\n[Error] GNINA failed on {filename}:\n{proc.stderr}", file=sys.stderr)
        return None
...
# ==========================================
# 3. GNINA実行のオーケストレーション
# ==========================================
def evaluate_results(protein_pdb, smiles_list, min_conf, min_cnn, num_workers):
    ...
    for i, smi in enumerate(smiles_list):
        ...
        for sdf in sdf_files:
            tasks.append((sdf, protein_pdb, f"complex_{i}", smi, min_conf, min_cnn))

    results = []
    
    # 改善点2 & OOM対策: ユーザーが指定したワーカー数で並列化
    actual_workers = min(num_workers, len(tasks)) if tasks else 1
    print(f"🚀 並列処理を開始します (指定ワーカー数: {num_workers}, 実際の起動数: {actual_workers})")
    ...
    if df.empty:
        print("\n⚠️ 警告: フィルタリングの条件が厳しすぎたか、GNINAの実行エラーによりすべてのポーズが除外されました。")
        return
...
if __name__ == "__main__":
    ...
    # ▼ 追加: GNINAの並列ワーカー数を指定（デフォルトはGPUに優しい1）
    parser.add_argument("-w", "--workers", type=int, default=1, help="GNINA並列実行時のワーカー数 (GPU使用時は1〜2を推奨。デフォルト: 1)")
    ...
    evaluate_results(args.p, smiles_list, args.mc, args.cp, args.workers)
