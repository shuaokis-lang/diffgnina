...
def evaluate_chunk_results(protein_pdb, ligand_chunk, start_idx, min_conf, min_cnn, num_workers, max_poses, out_dir="results", output_csv="filtered_results.csv"):
    print(f"\n⚖️ 評価・最適化中 (Index {start_idx} 〜, 並列数: {num_workers}, 評価ポーズ数: 各上位 {max_poses})...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-dp", type=int, default=20, help="DiffDockでの生成ポーズ数")
    parser.add_argument("-mp", type=int, default=5, help="GNINAで評価するDiffDockの上位ポーズ数 (Rank順)")
    ...
    for chunk_start in range(args.start_idx, total, args.chunk_size):
        ...
        evaluate_chunk_results(args.p, chunk_ligands, chunk_start, args.mc, args.cp, args.workers, args.mp, out_dir="results", output_csv=output_csv)
